...
<table border="0" class="data-table">
	...
	?>
	<tr>
		<!--Date field in the drill down table-->
		<td><?php echo date("d/m/Y",strtotime($disbursement->Date_Issued));?></td>
		<!--End of date cell-->
		<?php
  			//if the drugs are isseud by the national store then display Nationa store
			if($disbursement->Issued_By_National == "0")
			{ ?>
			<td> National Store </td> 
			<td style="color: green">
				<?php 
				//adds the amount of drugs issued by the store to the counter/varialble
				//vaccine totals
				$vaccine_totals +=$disbursement->Quantity; 
				//echos the disbursements isued by that respective store
				echo $disbursement->Quantity?>
				
			</td>
			<?php 
			}
			//end of if-statement if($disbursement->Issued_By_National == "0")		
 
			else if($disbursement->Issued_By_Region != null)
			{?>
				<td>
					<?php 
						//appends the name of the particular region to Regional Store
						echo $disbursement->Region_Issued_By->name
					?> Regional Store</td>
				<td style="color: green">
					<?php 
					$vaccine_totals +=$disbursement->Quantity; 
		...
		<?php } //end of else if-statement else if($disbursement->Issued_By_Region != null)
  
			else if($disbursement->Issued_By_District != null)
			{?>				
				<td>
					<?php 
					echo $disbursement->District_Issued_By->name?> 
					District Store</td>
				<td style="color: green">
					<?php 
					$vaccine_totals +=$disbursement->Quantity; 
		...
		<td>
			<?php 
				echo $disbursement->Voucher_Number;
			?>
		</td>
			<?php 
				if($disbursement->Batch_Number != null)
				{?>
	...
	<tr>
		...
		<td colspan="8" style="font-weight:bold">
			<?php echo $current_stocks[$vaccine->id] - $vaccine_totals?>
		</td>
	</tr>
 <?php 
	}
			...
			<div id="additional">
				<table border="0" class="data-table">
					...
					<tr>
						<td>Stock at Hand</td>
						<td>
							<?php echo $current_stocks[$vaccine->id];?> 
							Doses
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							...
							echo ceil($monthly_requirement);?> Doses
						</td>
					</tr>
					<tr>
						...
						<td>
							<?php 
							$population = str_replace(",","",$population); 
							echo number_format(($current_stocks[$vaccine->id]/$monthly_requirement),1);?> Month(s)
						</td>
					</tr>
					<tr>
						<td>Stock Received this Year</td>
						<td>
							<?php echo $vaccine_totals?> Doses
						</td>
					</tr>
					<tr>
					<td>Estimated Coverage</td>
						<td>
							...
							echo ceil(($vaccine_totals/$yearly_requirement)*100)?>%
						</td>
					</tr>
					<tr>
						<td>Supply Graph</td>
						<td>
							<a href="#" class="link supply_graph" vaccine = "<?php echo $vaccine->id;?>" >Click to View Graph</a>
						</td>
					</tr>
</table>
<div id="graph_container_<?php echo $vaccine->id;?>" title="Supply Graph for <?php echo $vaccine->Name;?> in <?php echo $recipient;?>" >
	<div  id="filter_<?php echo $vaccine->id;?>" style="display: none"> Select Filter Year:<select class="year_filter" vaccine = "<?php echo $vaccine->id;?>">
		<?php
		//Display options for up to 5 years ago 
		$start_year = date('Y');
