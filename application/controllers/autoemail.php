...
class autoemail extends MY_Controller 
{
	...
	public function printReport($from, $to) 
	{
		...
		foreach ($user_groups as $user_group) 
		{
			...
			{
				//picks all the provinses from the provinces table
				//they are 9 in total
				...
				foreach ($provinces as $province) 
				{
					...
					foreach ($vaccines as $vaccine) 
					{
						...
						@$sql_consumption = "select (SELECT max(str_to_date(Date_Issued,'%m/%d/%Y'))  
											...
											and str_to_date(Date_Issued,'%m/%d/%Y')between str_to_date('" . $start_date . "','%m/%d/%Y') 
