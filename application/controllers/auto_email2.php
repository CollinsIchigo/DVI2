...
class auto_email2 extends MY_Controller 
{
	...
	{
		@$this -> load -> database();
		//gets all the user groups from the db..they are 5 in total
			...
			if ($identifier == 'national_officer') 
			{
				...
				foreach ($provinces as $province) 
				{
					...
					foreach ($vaccines as $vaccine) 
					{
						...
						@$owners = Disbursements::getDistinct_owners;
						foreach ($owners as $owner)
						{
							@$sql_consumption = "select (SELECT max(str_to_date(Date_Issued,'%m/%d/%Y'))  
											...
											between str_to_date('" . $start_date . "','%m/%d/%Y') 
												...
												between str_to_date('" . $start_date . "','%m/%d/%Y') 7
												and str_to_date('" . $end_date . "','%m/%d/%Y') 
  ...
  str_to_date('" . $end_date . "','%m/%d/%Y') and Vaccine_Id = '" . $vaccine -> id . "')as total_received";
			...
			if ($identifier == 'national_officer') 
			{
				@$store = Regions::getRegionNational();
				@$data_buffer = "
					<style>
							table.data-table {
							table-layout: fixed;
				...
				foreach ($vaccines as $vaccine) 
				{
					...
					@$sql_consumption = "select (SELECT max(str_to_date(Date_Issued,'%m/%d/%Y'))  
										FROM `disbursements` where Owner = 'N0' 
										and str_to_date(Date_Issued,'%m/%d/%Y') 
										between str_to_date('" . $start_date . "','%m/%d/%Y') 
										and str_to_date('" . $end_date . "','%m/%d/%Y') 
										and Vaccine_Id = '" . $vaccine -> id . "' 
	...
	{
		$html_title = "<img src='Images/coat_of_arms-resized.png' 
						style='position:absolute; width:96px; height:92px; top:0px; left:0px; '>
						</img>";
